...
`Legs` keeps track of the list of recent coordinates that have been visited, the directions to get there, and the available steps.

The general strategy is to perform a breadth-first search from both the start and destination coordinates and to try out each step still available (ie not yet used up from the direction limits) to find where they may meet.
